...
alert("please connect wallet")
    ...
    if(item != 0){
            ...
            parentreferal = await contractCall.methods.getParentRefer(ids, item).call()
    }
    if (isConnected) {
        if (item == 0) {

            parentreferal = await contractCall.methods.getParentRefer(ids, address).call()
        } else {
            parentreferal = await contractCall.methods.getParentRefer(ids, item).call()
        }
...
if(isConnected){
    ...
    try {
      ...
      if (isConnected) {
          if (item == 0) {
              ...
              parentreferal = await contractCall.methods.getParentRefer(id, address).call()
          } else {

              parentreferal = await contractCall.methods.getParentRefer(id, item).call()
          }
  ...
  const setDisplayDetail = async (datas) => {
    ...
    for (let i = 0; i < datas.length; i++) {
      ...
      if (datas[i] != "0x0000000000000000000000000000000000000000") {
       let userData = await contractCall.methods
        ...
        let arr = {
          ...
          list: [
            {
              text: `${datas[i].toString().substring(0, 4)}...${datas[i]
                .toString()
                .substring(datas[i].length - 4, datas[i].length)}`,
            },{
  ...
  return (
    <div className="page-roadmap">
      <PageTitle title="Uplines" />
     
      <RoadMap
         data={data.length <= 0 ? [newuser] : data}
       // data={dataRoadmap}
        
        SubmitId={getId}
        SubmitSearch={getSearch}
      />
